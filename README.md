...
Settings → Items → Shortcuts lets you add visual launcher items for websites, app deep links, applications, files, and folders. The shortcut editor includes presets such as Outlook Calendar, which uses Outlook's native View → Go To → Calendar action because Outlook's `ms-outlook://` calendar deep links are unreliable on macOS. Shortcuts also support target pickers for apps/files/folders, URL validation, optional borrowed app icons by bundle ID, reordering, editing, and removal.
